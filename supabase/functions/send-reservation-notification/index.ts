...
function generateEmailHTML(params: ReservationNotificationRequest): string {
    ...
    const headerColor = isPending ? "#f59e0b" : "#10b981";
    const headerTitle = isPending ? "Yeni Rezervasyon Talebi" : "Rezervasyon Onaylandi";
    const statusText = isPending ? "onay bekliyor" : "onaylandi";
    ...
    return `<!DOCTYPE html><html lang="tr"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"></head><body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;line-height:1.6;color:#333;max-width:600px;margin:0 auto;padding:20px;"><div style="background:linear-gradient(135deg,${headerColor} 0%,${headerColor}dd 100%);padding:30px;border-radius:10px 10px 0 0;"><h1 style="color:white;margin:0;font-size:24px;">${headerTitle}</h1></div><div style="background:#f9fafb;padding:30px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 10px 10px;"><p style="margin-top:0;">Merhaba,</p><p><strong>${requester.name}</strong> tarafindan olusturulan bir rezervasyon <strong style="color:${statusColor};">${statusText}</strong>.</p><div style="background:white;padding:20px;border-radius:8px;border:1px solid #e5e7eb;margin:20px 0;"><h2 style="margin-top:0;color:${headerColor};font-size:18px;">${reservation.title}</h2><table style="width:100%;border-collapse:collapse;"><tr><td style="padding:8px 0;color:#6b7280;width:120px;">Oda:</td><td style="padding:8px 0;font-weight:500;">${reservation.roomName}</td></tr><tr><td style="padding:8px 0;color:#6b7280;">Baslangic:</td><td style="padding:8px 0;font-weight:500;">${startDate}</td></tr><tr><td style="padding:8px 0;color:#6b7280;">Bitis:</td><td style="padding:8px 0;font-weight:500;">${endDate}</td></tr><tr><td style="padding:8px 0;color:#6b7280;">Talep Eden:</td><td style="padding:8px 0;font-weight:500;">${requester.name} (${requester.email})</td></tr>${cateringRow}${recurringRow}${descriptionRow}</table></div>${isPending ? `<div style="background:#fef3c7;padding:15px;border-radius:8px;border-left:4px solid #f59e0b;margin:20px 0;"><strong>Bilgi:</strong> Bu rezervasyon admin onayi beklemektedir.</div>` : `<div style="background:#d1fae5;padding:15px;border-radius:8px;border-left:4px solid #10b981;margin:20px 0;"><strong>Bilgi:</strong> Bu rezervasyon onaylandi ve takvime eklendi.</div>`}<hr style="border:none;border-top:1px solid #e5e7eb;margin:20px 0;"><p style="color:#6b7280;font-size:14px;margin-bottom:0;">Bu email Ferko Space Manager tarafindan otomatik olarak gonderilmistir.</p></div></body></html>`;
}
...
serve(async (req) => {
    ...
    try {
        ...
        const subject = notificationType === "pending"
            ? `Yeni Rezervasyon Talebi: ${reservation.title}`
            : `Rezervasyon Onaylandi: ${reservation.title}`;
